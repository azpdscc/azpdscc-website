...
export function RaffleRegistrationForm() {
  ...
  return (
    <>
      <div className="p-6 rounded-lg border-primary/20 bg-primary/5 text-center">
        <div className="mx-auto bg-primary/10 rounded-full p-4 w-fit mb-4">
            <Ticket className="h-10 w-10 text-primary" strokeWidth={1.5} />
        </div>
        <h2 className="font-headline text-3xl text-foreground">Get Your Raffle Tickets via Text</h2>
        <p className="mt-2 text-muted-foreground">Enter your details below to receive your electronic raffle tickets directly to your phone.</p>
        
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-6 max-w-lg mx-auto">
                <div className="flex flex-col sm:flex-row gap-2">
                    <FormField name="name" control={form.control} render={({ field }) => (
                        <FormItem className="flex-1"><FormControl><Input placeholder="Your Name" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField name="phone" control={form.control} render={({ field }) => (
                        <FormItem className="flex-1"><FormControl><Input type="tel" placeholder="Your Phone Number" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                </div>
                ...
                <FormField
                    ...
                    <FormItem className="flex flex-col gap-3 rounded-md border p-4 bg-background">
                        ...
                        <div className="text-xs ml-7 text-left">
